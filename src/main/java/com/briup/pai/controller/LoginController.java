...
public class LoginController {
     ...
     * 因为此处是jwt的弊端,jwt一旦生成,无法销毁,只能等自动过期
     *
     * 通过后端接口请求,如果令牌时间很长,1个月
     *  登录获取令牌后,记录下来 令牌不要太长时间
     *  方案1: 将令牌存到redis中,redis的key的期限就是token期限
     *         注销用户,将redis中的token删除
     *         访问资源,比对redis中的令牌
     *         通过这种方式解决jwt无法销毁的问题
     *  方案2: 不用jwt,换别的token  oatuh2框架支持内置token可以销毁[不能存id信息]
     *         登录成功,将用户信息存到threadlocal
     *         获取用户身份,从threadlocal直接取出来
     *
     * 将令牌放到请求头携带到后端
     *  请求行
     *  请求头
     ...
     * 前端共享多次请求的资源 sessionStorage,localStorage Cookie
    ...
    //MultipartFile 上传excel
//    public void test(HttpServletResponse response, MultipartFile mf){
//
//    }
}